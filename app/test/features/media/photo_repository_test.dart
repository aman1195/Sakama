...
void main() {
  ...
  group('the object path, which IS the server-side boundary', () {
    test('is <user_id>/<something>.jpg', () {
      final p = repo.pathFor(userId: 'u-1')!;
    ...
    test('is always .jpg, because we declare the MIME type ourselves', () {
      ...
      expect(repo.pathFor(userId: 'u-1')!.endsWith('.jpg'), isTrue);
    });

    test('two photos never collide', () {
      final a = repo.pathFor(userId: 'u-1');
      final b = repo.pathFor(userId: 'u-1');
      expect(a, isNot(b));
    });
    ...
    test('there is NO path for a signed-out user', () {
      ...
      expect(repo.pathFor(userId: null), isNull);
      expect(repo.pathFor(userId: ''), isNull);
    });
  });
  ...
  test('a queued photo produces no crud op, so it cannot wedge sync', () async {
    // pending_uploads is local-only. The rejected-write outage that lost three
    // weeks of food logs came from a synced table; this one cannot repeat it.
    final f = await aFile();
    await repo.enqueue(localPath: f.path, kind: PhotoKind.progress, userId: 'u-1');
    expect((await repo.pending()).length, 1);
  });
